...
//AM: modified on 27/11
/*void LrWpanHelper::PrintRoutingTableAllAt(NodeContainer c ,Time printTime,Ptr<OutputStreamWrapper> stream, Time::Unit unit)
{
  for (NodeContainer::Iterator i= c.Begin(); i != c.End (); i++)
  {
    Ptr<Node> node = *i;    
    Simulator::Schedule (printTime, &LrWpanHelper::Print,node, stream, unit);
  }
}
void LrWpanHelper::Print(Ptr<Node> node,Ptr<OutputStreamWrapper> stream, Time::Unit unit)
{
  node->GetObject<LrWpanMac> ()->PrintRoutingTable(node,stream, unit);
}*/
